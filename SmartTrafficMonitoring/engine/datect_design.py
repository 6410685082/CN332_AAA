    ...
    def line_enter_check_and_set(loop,track,tp1,tp2,line_start,line_end):
        if isIntersect(tp1,tp2,line_start,line_end):
            if loop["id"] not in track.aoi_entered :
                ...
                Loop.append_to_file(str(save_dir)+"\\loop.txt",msg)
    ...
    def check_enter_exit_loop(track):
        ...
        for loop in loops:
            ...
            if len(track.centroidarr)>20:
                ...
                #check entering line
                Loop.line_enter_check_and_set(loop,track,tp1,tp2,pt0,pt1)
                
                #check exit line left straight and right
                Loop.line_exit_check_and_set(loop,track,tp1,tp2,pt1,pt2,"left")
                Loop.line_exit_check_and_set(loop,track,tp1,tp2,pt2,pt3,"straight")
                Loop.line_exit_check_and_set(loop,track,tp1,tp2,pt3,pt0,"right")
